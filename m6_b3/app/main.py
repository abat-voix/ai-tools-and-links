...
async def lifespan(app: FastAPI):
    ...
    # Phoenix-трейсинг (опциональный): инструментируем LlamaIndex до сборки
    # RAG-индекса, чтобы спаны покрывали и индексацию, и запросы.
    ...
    # Redis — опционален: при недоступности сервиса продолжаем без кеша.
    ...
    try:
        ...
        logger.warning("RAG/индексация не инициализированы (%s) — /rag/query и /documents вернут 503", e)

    # Агентный слой (LangGraph): ReAct-граф поверх RAG как инструмента.
    # Импорт ленивый — langgraph/langchain не нужны тестам без lifespan.
    app.state.agent_graph = None
    try:
        from langchain_openai import ChatOpenAI

        from app.agents.graph import build_custom_graph
        ...
        app.state.agent_graph = build_custom_graph(agent_model, agent_tools)
        logger.info("Агентный граф собран (%d инструмента)", len(agent_tools))
    except Exception as e:
        logger.warning("Агентный граф не собран (%s) — /agent/chat вернёт 503", e)
